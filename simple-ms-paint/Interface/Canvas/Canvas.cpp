...
void Canvas::onMouseHold(Vector2f mousePosition, vector<Button*> enabledButtons) {
	...
	for (auto& button : enabledButtons) {
		ButtonClass buttonClass = button->getButtonClass();
		if (buttonClass == ButtonClass::color)
			colorButton = static_cast<ColorButton*>(button);
		else if (buttonClass == ButtonClass::tool)
			toolButton = static_cast<ToolButton*>(button);
	}
